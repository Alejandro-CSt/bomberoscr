...
/**
 * Get highlighted incidents for a specific station, sorted by total emergency response deployment
 *
 * Returns incidents from the specified time range in which the provided station
 * was dispatched (regardless of whether it was responsible), each with a calculated
 * `totalDispatched` field representing the sum of dispatched vehicles and stations.
 * Results are ordered by highest deployment first.
 */
export async function getStationHighlightedIncidents({
  ...
  // Incidents where this station was dispatched (support or responsible)
...
export type StationHighlightedIncident = Awaited<
  ReturnType<typeof getStationHighlightedIncidents>
>[number];
