import { StatCard, StatCardSkeleton } from "@/features/dashboard/homepage/components/stat-card";
import { Skeleton } from "@/features/shared/components/ui/skeleton";
import { getYearRecap } from "@bomberoscr/db/queries/homepage/yearRecap";
import { cacheLife } from "next/cache";
import { Suspense } from "react";

async function YearRecapContent() {
  "use cache";
...
export function YearRecapHero() {
  return (
    <div className="relative flex select-none flex-col gap-4 rounded-lg py-4">
      <Suspense fallback={<YearRecapSkeleton />}>
        <YearRecapContent />
      </Suspense>
    </div>
  );
