...
export function StationHighlightedCard({ incident }: { incident: StationIncident }) {
  ...
  return (
    <Link
      href={incident.url}
      className="group flex overflow-hidden rounded-lg bg-card transition-colors hover:bg-accent/10"
    >
        ...
        {hasMap ? (
          <div className="h-full w-full overflow-hidden rounded-md">
            <Image
              ...
              className="h-full w-full scale-125 object-cover transition-transform duration-500 ease-out group-hover:scale-150"
              loading="lazy"
              width={384}
              height={384}
              placeholder="blur"
          ...
          >
            <div className="w-fit rounded-sm bg-background/60 p-2 backdrop-blur-3xl">
              <TriangleAlertIcon className="shrink-0 text-amber-500" size={16} aria-hidden="true" />
            </div>
          </div>
        )}
      </div>
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-2 py-3 pr-4 pl-2">
        <h3 className="line-clamp-2 font-medium text-base leading-snug transition-colors group-hover:text-primary">
          {incident.details}
        </h3>
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-muted-foreground">
            <div className="flex items-center gap-1.5" title="Estaciones despachadas">
              <GarageIcon size={16} />
              <span className="font-medium text-sm">{incident.dispatchedStationsCount}</span>
            </div>
            <div className="flex items-center gap-1.5" title="Vehículos despachados">
              <FireTruckIcon size={16} />
              <span className="font-medium text-sm">{incident.dispatchedVehiclesCount}</span>
            </div>
          </div>
          <RelativeTime
            date={incident.incidentTimestamp}
            className="shrink-0 font-medium text-muted-foreground text-xs first-letter:uppercase"
          />
        </div>
      </div>
    </Link>
...
export function StationHighlightedCardSkeleton() {
  return (
