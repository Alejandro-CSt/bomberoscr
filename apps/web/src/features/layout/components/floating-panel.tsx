...
export function FloatingPanel({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AnimatePresence mode="sync">
      {pathname !== "/" && (
        <motion.div
          className="fixed top-20 bottom-0 z-50 max-h-dvh w-full overflow-hidden rounded-xl border bg-background shadow-xl max-md:rounded-b-none md:bottom-4 md:left-8 md:w-[450px] md:max-w-[calc(100vw-32px)]"
